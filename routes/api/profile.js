...
router.delete('/experience/:exp_id', auth, async (req, res) => { 
    try {
        const profile = await Profile.findOne({ user: req.user.id });
        const removeIndex = profile.experience.map(item => item.id.toString()).indexOf(req.params.exp_id);
        if (removeIndex === -1) {
...
router.delete('/education/:edu_id', auth, async (req, res) => { 
    try {
        const profile = await Profile.findOne({ user: req.user.id });
        const removeIndex = profile.education.map(item => item.id.toString()).indexOf(req.params.edu_id);
        if (removeIndex === -1) {
...
// @desc    Get github repositories by github username
// @access  Public
